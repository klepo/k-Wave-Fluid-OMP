...
%       'u_max', 'p_rms', 'I_avg') and the final quantities ('p_final',
...
%                             z-direction over the complete domain
%                             (returned if 'u_final' is set)  
...
%                             recorded at the sensor positions given by
%                             sensor.mask (returned if 'I_avg' is set)
%       sensor_data.Iz_avg  - average acoustic intensity in the z-direction
%                             recorded at the sensor positions given by
%                             sensor.mask (returned if 'I_avg' is set)
%
% ABOUT:
...
if record.u_final
    sensor_data.ux_final = ux_sgx(x1:x2, y1:y2, z1:z2);
    sensor_data.uy_final = uy_sgy(x1:x2, y1:y2, z1:z2);
    sensor_data.uz_final = uz_sgz(x1:x2, y1:y2, z1:z2);
end
...
if record.p_final
    sensor_data.p_final = p(x1:x2, y1:y2, z1:z2);
end
