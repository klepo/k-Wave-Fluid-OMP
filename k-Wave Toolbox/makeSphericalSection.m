function [ss, dist_map] = makeSphericalSection(radius, height, width, plot_transducer, binary)
...
%       a flat surface.
%
%       If the Boolean input parameter "binary" is set to false (the
%       default), the sphere map is returned as a double precision matrix.
%       If it is set to true, the map is returned as a logical matrix.
%
% SYNTAX:
...
%       ss = makeSphericalSection(radius, height, width, plot_transducer, binary)
...
%       [ss, dist_map] = makeSphericalSection(radius, height, width, plot_transducer, binary)
...
if nargin < 4 || isempty(plot_transducer)
