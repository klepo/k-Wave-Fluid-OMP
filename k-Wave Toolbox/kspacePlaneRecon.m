...
% order N, M, P
try 
    % try to use griddedInterpolant, but first remove '*' from input option
    if strncmp(interp_method, '*', 1)
        interp_method = interp_method(2:end);
        removed_asterisk = true;
    else
        removed_asterisk = false;
    end
    F = griddedInterpolant(w, kgrid.ky, kgrid.kz, p, interp_method);
    p = F(w_new, kgrid.ky, kgrid.kz);
catch ME
    % if the function doesn't exist, use interp3
    if strcmp(ME.identifier, 'MATLAB:UndefinedFunction')
        % add back the '*' if it was removed
        if removed_asterisk
