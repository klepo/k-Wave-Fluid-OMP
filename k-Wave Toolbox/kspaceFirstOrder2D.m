...
%         rhoy = bsxfun(@times, pml_y, ( bsxfun(@times, pml_y, rhoy) - dt.*(2*rhoy + rho0) .* duydy)); 
    ...
    if use_sensor && ~time_rev && (t_index >= sensor.record_start_index)
        ...
        if record.p_max_all
            if file_index == 1
                sensor_data.p_max_all = p(x1_inside:x2_inside, y1_inside:y2_inside);
            else
                sensor_data.p_max_all = max(sensor_data.p_max_all, p(x1_inside:x2_inside, y1_inside:y2_inside));
            end
        end
        ...
        if record.p_min_all
            if file_index == 1
                sensor_data.p_min_all = p(x1_inside:x2_inside, y1_inside:y2_inside);
            else
                sensor_data.p_min_all = min(sensor_data.p_min_all, p(x1_inside:x2_inside, y1_inside:y2_inside));
            end                
        end              
        ...
        if record.u_max_all
            if file_index == 1
                sensor_data.ux_max_all = ux_sgx(x1_inside:x2_inside, y1_inside:y2_inside);
                sensor_data.uy_max_all = uy_sgy(x1_inside:x2_inside, y1_inside:y2_inside);
            else
                sensor_data.ux_max_all = max(sensor_data.ux_max_all, ux_sgx(x1_inside:x2_inside, y1_inside:y2_inside));            
                sensor_data.uy_max_all = max(sensor_data.uy_max_all, uy_sgy(x1_inside:x2_inside, y1_inside:y2_inside));            
            end
        end        
        ...
        if record.u_min_all
            if file_index == 1
                sensor_data.ux_min_all = ux_sgx(x1_inside:x2_inside, y1_inside:y2_inside);
                sensor_data.uy_min_all = uy_sgy(x1_inside:x2_inside, y1_inside:y2_inside);
            else
                sensor_data.ux_min_all = min(sensor_data.ux_min_all, ux_sgx(x1_inside:x2_inside, y1_inside:y2_inside));            
                sensor_data.uy_min_all = min(sensor_data.uy_min_all, uy_sgy(x1_inside:x2_inside, y1_inside:y2_inside));                     
            end
        end 
...
if record.p_final
    sensor_data.p_final = p(x1_inside:x2_inside, y1_inside:y2_inside);
end

% save the final particle velocity if required
if record.u_final
    sensor_data.ux_final = ux_sgx(x1_inside:x2_inside, y1_inside:y2_inside);
    sensor_data.uy_final = uy_sgy(x1_inside:x2_inside, y1_inside:y2_inside);
end
