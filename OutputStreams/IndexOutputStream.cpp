...
IndexOutputStream::IndexOutputStream(Hdf5File& file,
  ...
  : BaseOutputStream(file, datasetName, sourceMatrix, reduceOp, bufferToReuse, outputStreamContainer, doNotSaveFlag),
    mSensorMask(sensorMask), mDataset(H5I_BADID), mSampledTimeStep(0)
{
...
void IndexOutputStream::create()
{
  ...
  mBufferSize = (mReduceOp == ReduceOperator::kC)
                  ? ceilf(mSensorMask.size() * mComplexSize) * mCompressHelper->getHarmonics()
                  : mSensorMask.size();
  ...
  if (!mDoNotSaveFlag)
  {
    ...
    {
      datasetSize = DimensionSizes(mBufferSize, params.getNt() - params.getSamplingStartTimeIndex(), 1);
    }
...
void IndexOutputStream::reopen()
{
  ...
  { // aggregated quantities - reload data
    ...
    if (params.getTimeIndex() > params.getSamplingStartTimeIndex())
    {
      ...
      mFile.readCompleteDataset(mFile.getRootGroup(), objectName, DimensionSizes(mBufferSize, 1, 1), mStoreBuffer);
    }
  }
