...
BaseOutputStream::BaseOutputStream(Hdf5File& file,
  ...
  bool doNotSaveFlag)
  : mFile(file), mRootObjectName(rootObjectName), mSourceMatrix(sourceMatrix), mReduceOp(reduceOp),
    mBufferReuse(bufferToReuse != nullptr), mBufferSize(0), mStoreBuffer(bufferToReuse),
    mOutputStreamContainer(outputStreamContainer), mDoNotSaveFlag(doNotSaveFlag)
{
...
void BaseOutputStream::loadCheckpointCompressionCoefficients()
{
  if (mReduceOp == ReduceOperator::kC)
  {
    ...
    checkpointFile.readCompleteDataset(
      checkpointFile.getRootGroup(), "Temp_" + mRootObjectName + "_1", DimensionSizes(mBufferSize, 1, 1), mStoreBuffer);
  ...
  if (mReduceOp == ReduceOperator::kIAvgC)
  {
    Hdf5File& checkpointFile = Parameters::getInstance().getCheckpointFile();
    checkpointFile.readCompleteDataset(
      checkpointFile.getRootGroup(), "Temp_" + mRootObjectName, DimensionSizes(mBufferSize, 1, 1), mStoreBuffer);
  }
