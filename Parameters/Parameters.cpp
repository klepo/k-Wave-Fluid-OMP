...
void TParameters::ReadScalarsFromHDF5InputFile(THDF5_File & HDF5_InputFile){
    ...
    if (absorbing_flag != 0) {
        ...
        if (alpha_coeff_scalar_flag) HDF5_InputFile.ReadCompleteDataset(HDF5RootGroup, alpha_coeff_Name, ScalarSizes, &alpha_coeff_scalar);                
    }
    
       
    c0_scalar_flag = HDF5_InputFile.GetDatasetDimensionSizes(HDF5RootGroup, c0_Name) == ScalarSizes;
    if (c0_scalar_flag) HDF5_InputFile.ReadCompleteDataset(HDF5RootGroup, c0_Name, ScalarSizes, &c0_scalar);                
    
    if (nonlinear_flag) {
        BonA_scalar_flag = HDF5_InputFile.GetDatasetDimensionSizes(HDF5RootGroup, BonA_Name) == ScalarSizes;
        if (BonA_scalar_flag) HDF5_InputFile.ReadCompleteDataset(HDF5RootGroup, BonA_Name, ScalarSizes, &BonA_scalar);                    
    }
